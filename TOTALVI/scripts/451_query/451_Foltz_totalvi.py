...
adata_ref = anndata.concat([adata_730,adata_3228])
...
adata_ref = adata_full[np.logical_or(adata_full.obs.batch == "730", adata_full.obs.batch == "3228")].copy()
...
print("\n Total 451 query train time: {}".format(end-start))

dir_path = "query_model_451" + "/"
vae_q.save(dir_path, overwrite=True)

record = open("record.txt", "a")  # append mode
record.write("3228 and 730 query written at ")
record.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
record.write("\n 451 query train time: {}".format(end-start))
record.write("\n")
record.close()
